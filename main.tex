...
\todo[inline]{Peut-être ajouter + de détails sur le contexte du travail et son but?}
\newpage
...
Figure \ref{fig:vib_comp} presents the radiated powers in the Far Field (at 10m) of the speaker and speaker + wood measurements. Here the effect of the wooden plate vibrating can be observed clearly: the radiated power has a lot of oscillations on the speaker + wood measure, which are not found on the speaker only measure. At the resonance (f = 150 Hz) a strong cancellation effect is observed, with a difference of 6 dB between the 
\todo[inline, color=red!40]{This needs to be rephrased}
...
\todo[inline, color=red!40]{Make a better explanation}
...
The major mismatches between the free air and voltage compensated curves can be explained by the approximations made on the box's behavior. A LEM for a sealed box can be much more complicated than a change on the mechanical compliance 
...
\todo[inline, color=red!40]{mieux formuler + tirer des conclusions et expliquer les autres figures}
\begin{minipage}{0.35\textwidth}
\begin{center}
...
\todo[inline]{Finir le code et créer la courbe de correction générique}
