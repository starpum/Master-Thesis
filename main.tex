...
\todo[inline, color=red!40]{FINIR DE REDIGER}
...
The method have some serious drawbacks, mainly it only works on-axis and the absolute level isn't good.

\todo[inline, color=red!40]{faire la far field method pour la mesure précédente et comparer les résults}


\section{Measurements in the tetrahedral box}
Although the previous methods are effective, they require the reference curve to be remeasured for each environment or change of the DUT position, because the room effects depends on the positioning of the loudspeaker. A solution to this problematic is to measure in test boxes, such as the tetrahedral test box created by Hill Acoustics \citep[see][]{tetbox}: the boxes provide a shielding against the outside environment, allowing a reproductibility of results. 
...
Figure \ref{fig:MicPosDiffRC} shows the difference between the Room Compensation functions presented in figure \ref{fig:MicPosDiffNFS}. If the microphone position had no influence on the measurement, the difference should be zero.


 In this frequency range, the accepted level of differences is $\pm$ 5 dB. The distances $D_{ms}$ satisfying that range are 5 cm, 6 cm and 7 cm. When the microphone is 8 cm away from the speaker the differences are already too critical to ensure an accurate measurement. \\
...
\begin{minipage}{0.5\textwidth}
\begin{center}
	\includegraphics[width=0.8\textwidth]{RoomComp/diff_kms} 
    \captionsetup{hypcap=false} 
