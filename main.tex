...
\begin{center}
	\includemovie[draft,repeat,poster,label=baffle_vib,text={\small(Loading Video...)}]{6cm}{4cm}{GridOpti/baffle_vib2.avi}
\end{center}
...
%presents the contour plot of the speaker and the speaker+wood measurements, for a frequency range of 20Hz - 5kHz and angles from -180° to 180°. This plots are very similar, however the measurement of speaker + wood presents much more fluctuations on the side lobes and the DUT stops emitting as a monopole sooner than the speaker alone: at around 770Hz for the speaker alone, and 350Hz for the speaker + wood.\\
...
To correct the effect of the box, an input voltage for which $x(\omega )$ = $x_{b}(\omega ) $ can be calculated as given in equation \ref{eq:V_corr} where $K_{ms}$ and $K_{msb}$ are the stiffness in free air and in the box respectively, and $K_{ms}$ = $\frac{1}{C_{ms}}$.
...
Now that the theory is expressed, this assumptions needs to be verified. To do so, several measurements have been performed on the 10 cm woofer (see appendix \ref{spkrlib:10cm}).
