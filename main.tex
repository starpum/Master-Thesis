...
If the field separation cannot be applied on the measurement; using the elliptical grid can greatly improve the results: moving the microphone closer to the DUT will make the final results more accurate thanks to the robustness to room mode mentioned previously. \\
...
Another investigation have been performed to identify the rattling generated by the clamping system (see picture in appendix \ref{Chap:baffle_pics}): if the nuts aren't tighten enough, an audible rattling sound is emitted and will pollute the measurement. Figure \ref{fig:sweep_baffle} presents the time signal measured by a microphone on the upper left corner of the baffle

\begin{center}
	\includegraphics[width=0.5\textwidth]{GridOpti/Baffle_Rattle} 
    \captionsetup{hypcap=false} 
...
\todo[inline]{say the plate vibration probs can be reduced by a smaller radius, but will never be totally eased off because the speaker's mounting system vibrates too and it would be stupid to make a meas IN the speaker} 
...
For the baffle and the dual plane symmetries, the same principle as for the single plane symmetry is applied to retrieve the coefficient matrix. Of course, the the indexes of the coefficients. There is no need the multiply the right-side matrix by the weights matrix $R_{m}$ for those symmetries, as seen in subsection \ref{chap:sym}. \\
...
The symmetry factor can be expressed in dB, and allows to quantify the differences between the total radiated power and the symmetry radiated power. The symmetry presenting the less differences is called the "dominant symmetry". \\
...
\begin{description}
\item[Figure \ref{fig:comp_meas}] presents the measurements without any compensation applied. The decay of the room have a large impact on the measurement, and makes the speaker's behavior hard to discriminate from the room's artifacts. In low frequencies, the modes are clearly visible and spreaded and will have a big impact on the measurement because they'll be there all the time. In high frequencies, the modes could be eased-off by smoothing the TRF.

\item[Figure \ref{fig:comp_mag}] a pre-filter have been applied on the magnitude of the TRF, using the method shown in figure \ref{fig:flow_postfilt}, where the filter $H_{c}$ is the difference between a reference measurement done with the NFS and the TRF measurement. This method is the most straight-forward to implement, but the filter $H_{c}$ has a zero phase and is only applied on the magnitude. The lack of phase information generates the acausality observed on the TFA.
...
The Low-Frequency Reference method gives accurate results, but requires a good reference curve to be relevant. In the following, different methods of obtaining the reference curve and different measurement environments 
...
\todo[inline, color=red!40]{expliquer vite fait principe correct LF, et passe à DB keele+ dire que ca amrche que pour 1 driver}

The results shows a good match before the cross-frequency range (shown by the vertical yellow bars in figure \ref{fig:resNFFF}) 
...
Although the previous methods are effective, they require the reference curve to be remeasured for each environment or change of the DUT position, because the room effects will change. A solution to this problematic is to measure in test boxes, such as the tetrahedral test box created by Hill Acoustics \citep[see][]{tetbox}: the boxes provide a shielding against the outside environment, allowing a reproductibility of results. For example measuring
...
\todo[inline]{Expliquer que l'on peut utiliser ca pour compenser l'effet de la boite dans le domaine linèaire}
